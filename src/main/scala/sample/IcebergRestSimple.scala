...
//      .config("spark.sql.catalog.rest_catalog.s3.secret-key-id", Constants.s3_secret_key)
//      .config("spark.sql.catalog.rest_catalog.s3.secret-access-key", Constants.s3_access_key)
//      .config("spark.executor.extraJavaOptions", String.format("-Daws.region=%s", Constants.s3_region))
      .config("spark.sql.catalog.rest_catalog.client.credentials-provider", "sample.CustomCredentialProvider")
      // https://github.com/apache/iceberg/issues/10078
//      .config("spark.sql.catalog.rest_catalog.client.credentials-provider", "software.amazon.awssdk.auth.credentials.SystemPropertyCredentialsProvider")
      .config("spark.sql.catalog.rest_catalog.client.region", "us-east-1")
      .config("spark.sql.catalog.rest_catalog.client.credentials-provider.accessKeyId", Constants.s3_access_key)
      .config("spark.sql.catalog.rest_catalog.client.credentials-provider.secretAccessKey", Constants.s3_secret_key)
