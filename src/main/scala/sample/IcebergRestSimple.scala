...
//      .config("spark.sql.catalog.rest_catalog.warehouse", "s3://" + Constants.s3_bucket + "/iceberg") // S3 存储路径
      ...
      .config("spark.sql.catalog.rest_catalog.s3.access-key-id", Constants.s3_access_key)
      .config("spark.sql.catalog.rest_catalog.s3.secret-access-key", Constants.s3_secret_key)
